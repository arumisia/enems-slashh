 ...
 */
@Mod.EventBusSubscriber(modid = Enems_slashh.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE, value = Dist.CLIENT)
public class PlayerAnimationTrigger {
    ...
    public static void onChatReceived(ClientChatReceivedEvent event) {
        ...
        if (event.getMessage().contains(Component.literal("waving"))) {
            ...
            var animation = (ModifierLayer<IAnimation>)PlayerAnimationAccess.getPlayerAssociatedData((AbstractClientPlayer) player).get(new ResourceLocation(Enems_slashh.MODID, "animation"));
            if (animation != null) {
                //You can set an animation from anywhere ON THE CLIENT
                //Do not attempt to do this on a server, that will only fail

                animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation("enems_slashh", "waving"))));
